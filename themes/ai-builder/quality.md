...
9. **Internal contradiction within one item**。寫完每個 item 後回頭讀一次：
   ...
   - 一個 item 的 market evidence 說「3 independent signals」，這 3 個 signal 的 upstream 是同一個事件嗎？一個論文作者在 HN 宣傳之後的漣漪**不是** three convergent signals。
...
#### Grounding / 反幻覺規則（連接組織層，2026-05 連續復發的失效模式）

這些不是文風問題，是事實忠實度問題：

- **時間熔接**：除非兩個來源的 `source-ages.json` age 都 ≤ 7，否則不得寫「同週 / 同時發布 / 同一週 / 今天匯流」。年齡用查的，不要用估的。把不同時間的來源說成「同一週的三個訊號」是本報告最常見的幻覺。
- **具名誤植**：把「某作者整理了 X」寫成「某作者確認 X 已在生產量產」是捏造。具名歸因只能斷言該來源 `takeaway` 字面有的東西——不得自行加上「確認 / 已量產 / 因此導致 / N 倍」。
- **數字幅度**：除非來源明載，不得宣稱倍率（`N 倍 / Nx`）；廠商的 "up to" 行銷倍率是宣稱不是事實，優先用絕對數字。
- **寧缺勿濫**：當天沒有真正的跨來源匯流時，寫更少的 signal 或明說「無匯流」，不要為了填滿版位而硬湊。
...
但要注意：**5 分鐘咖啡 brief 的軟性目標還在**。外審 PM 說上一版讀 lead + signals 要 12 分鐘，signals / predictions 完全跳過。如果你的 lead 超過 3500 字 HTML 或者 predictions 超過 7 條，你很可能在 padding——回去砍一段，或改成更精簡的描述。
