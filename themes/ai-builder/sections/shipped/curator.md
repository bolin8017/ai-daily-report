...
- `data/staging/feeds-shipped.json` — one object with `trending`, `search`, and `developers` arrays
...
Source: `feeds-shipped.json` `trending` array. Pick AI/ML and AI-builder-adjacent infra.
...
Source: `feeds-shipped.json` `search` array (already filtered to ≤30-day-old AI repos with ≥100 stars).
...
Source: `feeds-shipped.json` `developers` array, items where `developer_region === 'global'` (top 100, ≥1000 followers).
