...
本文件列出本專案每日爬取/抓取的所有來源，以 `src/fetchers/` 下的 4 支 fetcher 為分組單位。配置的權威來源是 [`config.json`](../config.json)。

> **Last verified against `config.json`: 2026-05-22** — 與 config.json 對齊的最近確認日期。修改 config 或本文件時請一併更新此日期。

> 更新規則：新增或調整來源時，請同步修改 `config.json` 並更新本表，然後執行 `npm run check:sources` 驗證沒有漏。
...
| Fetcher | 對應檔案 | 抓取機制 |
|---|---|---|
| `feeds` | `src/fetchers/feeds.js` | RSSHub routes / 原生 RSS / 原生 JSON API（依 `config.json.sources.feeds[].type` 分派） |
...
RSSHub fallback：`config.json.sources.rsshub_urls` 為一組依序嘗試的公共實例，目前順序為：
...
預設啟用 35 個來源（不含 lens overlay）。依分類整理如下。
...
> Phison Blog / vLLM Releases / LMCache Releases / NVIDIA Developer / SK Hynix News / BlocksAndFiles 透過 lens overlay 提供，見 §5。
...
每個 topic 上限 10 筆（`config.json.sources.github_topics.limit_per_topic`），結果用於 `shipped` 的 discovery picks（每日 3–5 個）。
...
新 repo 視窗：72 小時（`config.json.sources.github_developers.new_repo_window_hours`）。輸出對應到報告中的 `dev_watch.taiwan` / `dev_watch.global`。

---

## 5. Lens 覆蓋：Phison aiDAPTIV+

`config.json.lenses[].sources_overlay` 在執行該 lens 時會「附加」到預設來源之上（不取代）。目前僅 `phison-aidaptiv` 有 overlay：
...
執行下列指令可一次比對 `config.json` 與本文件是否同步（包含 lens overlay）：

```bash
npm run check:sources
```

- 通過：輸出 `OK — all N sources from config.json present in docs/data-sources.md`，退出碼 0。
- 漂移：列出 `config.json` 內存在但本文件未提到的每一個 `name` / `topic` / 區域，退出碼 1。
- 邏輯實作見 [`scripts/check-data-sources.mjs`](../scripts/check-data-sources.mjs)。腳本只檢「config → doc」單向漂移，因為本文件含大量自由文字，反向比對誤判率太高；如果你「刪除來源」，請手動同步移除本文件對應段落。
...
- **抓取頻率**：所有來源每天 04:00 Asia/Taipei 抓一次（systemd timer + Docker）。
