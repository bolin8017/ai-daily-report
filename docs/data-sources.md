...
> **Last verified against `themes/ai-builder/sources.yaml`: 2026-07-24** — 與 sources.yaml 對齊的最近確認日期。修改 sources.yaml 或本文件時請一併更新此日期。
...
預設啟用 37 個來源（不含 Phison overlay）。依分類整理如下。
...
| 名稱 | URL | 分類 | 上限 |
|---|---|---|---|
| Google AI Blog | `https://blog.google/innovation-and-ai/technology/ai/rss/` | AI 公司 | 10 |
...
> Phison Blog / vLLM Releases / LMCache Releases / SK Hynix News / BlocksAndFiles 透過 theme 的 `phison_overlay` 提供，見 §5。

#### 1.3.7 圖像生成 / Diffusion（→ `tech.vendor`）
...
#### 1.3.9 政策法規（→ `market.policy`）

| 名稱 | URL | 上限 |
|---|---|---|
| Lawfare | `https://www.lawfaremedia.org/feeds/articles` | 5 |
| 國科會 NSTC | `https://www.nstc.gov.tw/nstc/rss/news.xml` | 5 |

#### 1.3.10 台灣媒體（→ `market.taiwan`）

| 名稱 | URL | 上限 |
|---|---|---|
| TechNews | `https://technews.tw/feed/` | 10 |
| Inside | `https://www.inside.com.tw/feed/rss` | 10 |
| TechOrange | `https://techorange.com/feed/` | 8 |
| EE Times Taiwan | `https://www.eettaiwan.com/feed/` | 10 |
| 鉅亨網科技 | `https://news.cnyes.com/rss/v1/news/category/tech` | 10 |
| 經濟日報產業 | `https://money.udn.com/rssfeed/news/1001/5591?ch=money` | 10 |
| T客邦 | `https://feeds.feedburner.com/techbang` | 8 |

> DIGITIMES 已於 2026-05-25 移除：其 RSS 端點 404、且網站封鎖爬蟲，每次抓取皆 0 筆。Inside 改用 native feed（舊 feedburner 端點失效）。

#### 1.3.11 MOPS 重大訊息（→ `market.taiwan`）

官方公開資訊觀測站重大訊息，依掛牌別分兩個 OpenAPI（欄位名不同，由 `mops-twse-openapi` provider 抓取後合併）：

| 掛牌別 | OpenAPI |
|---|---|
| 上市 TWSE | `https://openapi.twse.com.tw/v1/opendata/t187ap04_L` |
| 上櫃 TPEx | `https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap04_O` |

追蹤清單（群聯 8299 生態系；8299 / 3260 威剛 / 5274 信驊 為上櫃，故 TWSE-only 抓取會漏掉，需 TPEx）：

- **記憶體 / 控制 IC / 模組**：8299 群聯、2337 旺宏、2344 華邦電、2408 南亞科、3260 威剛、4967 十銓、8271 宇瞻、2451 創見
- **aiDAPTIV+ 夥伴與 AI server / PC**：2376 技嘉、3515 華擎、2395 研華、6166 凌華、2377 微星、2357 華碩、2353 宏碁、6669 緯穎、2382 廣達、3231 緯創、2356 英業達
- **晶圓 / 封測 / server 晶片**：2330 台積電、3711 日月光、2454 聯發科、5274 信驊
...
每個 topic 上限 10 筆（fetcher 內硬碼），結果用於 `shipped` 的 discovery picks（每日 3–5 個）。

Topics 由 `themes/ai-builder/interests.yaml`（interest-subscription registry）管理，每個 topic 設定 `level: core | rotating | off` 及對應的 `github:` 搜尋詞。`core` topics 每天全數查詢；`rotating` topics 每天依日期種子取樣（`rotation.rotating_per_day`）輪替，避免每日 query 量過大又能維持新鮮度覆蓋。請直接查閱該檔取得完整的 topic 清單。
...
| BlocksAndFiles | `https://www.blocksandfiles.com/?lab_viewport=rss` | ssd-vendor | 10 |
...
## 6. Leaderboards — 評測排行榜（event-driven）

排行榜由 `src/fetchers/providers/leaderboards-parsers/` 下各 parser 抓取，結果存入 `data/staging/` 並與 `data/leaderboard-snapshots.json`（跨日快照帳本）做 diff。**只有 top-5 排名實際改變時才輸出項目**（event-driven）；若本次抓取結果與前次快照相同，該排行榜靜默，不佔 `tech.benchmarks` 欄位。快照帳本由 Stage 4 commit 至 `data` branch，CI 於建構時 hydrate，生命週期與 `data/star-history.json` 相同。

| 名稱 | URL | 說明 |
|---|---|---|
| BFCL | `https://gorilla.cs.berkeley.edu/leaderboard.html` | Berkeley Function-Calling Leaderboard；靜默參考（僅在排名異動時輸出） |
| LMArena | `https://lmarena.ai/leaderboard` | Chatbot Arena ELO 排行（crowd-sourced）|
| LiveBench | `https://livebench.ai/` | 無污染動態題庫排行 |
| SWE-bench-Live | `https://swe-bench-live.github.io/` | 持續更新的 SWE-bench 子集 |
| GPQA Diamond (Epoch) | `https://epoch.ai/benchmarks/gpqa-diamond` | 博士級科學問答 |
| HLE (Epoch) | `https://epoch.ai/benchmarks/hle` | Humanity's Last Exam |
| tau2-bench | `https://taubench.com/` | Tool-use agent 基準測試 |
| GAIA | `https://huggingface.co/spaces/gaia-benchmark/leaderboard` | General AI Assistants，via parquet decode（`hyparquet`） |
| Artificial Analysis Intelligence Index | `https://artificialanalysis.ai/` | 綜合模型評比指數；需 `AA_API_KEY`，fail-soft（無 key 時靜默跳過） |

> **已移除的排行榜**：OCRBench（HF CSV 約 1 年未更新，現為 HTML-only）、Aider Polyglot（從未上線）、SWE-bench main board（JSON 資料約 4 個月未更新）。
