...
### Step 8: Emit the report JSON

`src/pipeline.js` captures your stdout, parses it as JSON, validates it against `src/schemas/report.js` (`ReportSchema`), and saves it to `data/reports/YYYY-MM-DD.json`. Your job is to emit exactly one JSON object matching that schema.

Required top-level keys: `date`, `lead.html`, `ideas[]`, `shipped[]`, `pulse`, `signals[]`.

Optional but rendered by the templates: `dev_watch`, `sleeper`, `contrarian`, `predictions`.

**Do not emit** `dashboard`, `tracked_topics`, `sources_status`, `memory_status` — these were legacy placeholders; no template renders them.

Do NOT wrap the JSON in code fences. Do NOT add preamble or explanation. Your entire response must be parseable by `JSON.parse` as a single object with no modifications.

### Step 9: (reference) Memory update rules — applied by a separate claude call

The pipeline makes a second `claude -p` call after your response to update `data/memory.json`. You do NOT need to produce memory output in this call — it would be discarded. The rules below describe what that second call will do, so you know how your output gets consumed downstream:

- New featured repos added to `short_term.featured_repos`; `times_featured` incremented for repeats.
- One new entry added to `short_term.key_observations` extracted from the lead.
- `topics` frequency updated (`hit_count`, `last_seen`, `first_seen`).
- `narrative_arcs`: a pattern on 2+ distinct days becomes an arc with `status: "emerging"`, `episode_count: 2`. 3+ days → `status: "active"`. 5 days idle → `stale`. Each arc MUST have both `id` (kebab-case slug) and `title` (Chinese phrase). Arc `id` slugs MUST NOT leak into reader-facing prose — if you need to mention an arc in your lead prose, use its `title`.
- Promotion: `short_term.featured_repos[*]` → `long_term.featured_repos` when `times_featured >= 3`.
- Prune: `short_term` older than 7d, `long_term` older than 30d, resolved predictions.

### Step 10: Stop

Do NOT attempt to run any commands, write any files, read any additional files, or do anything besides emit the JSON. The orchestrator handles schema validation, the 11ty build, and git commit/push.
...
- If memory is empty (first run): add 🚀 Launch Edition note at the top
...
| Symptom | Action |
|---|---|
| Some sources empty | Continue with available data. Degraded runs are fine; do not fabricate items to meet quotas. Mention the degraded state in the lead's `<h4>發生了什麼</h4>` paragraph if the absence is itself part of the story (e.g., "Anthropic News 今天全天無更新"). |
| All sources empty | Already handled by run.sh — won't reach you |
| Unsure about a field shape | Read `src/schemas/report.js` directly. Validation runs only after you finish, and a failure aborts the deploy with no chance to retry — verify before writing, not after. |
| The day feels genuinely thin | Fewer than ~80 unique non-duplicate candidates across all fetchers is "thin". Mark `lead.thin: true`, let the lead run short, drop shipped below 12 only if truly necessary. Don't pad with filler to hit arbitrary targets. |
| Watchlist phrases keep showing up in your draft | You are reaching for filler. Run the Step 7 slop test on each sentence — if it fails all 4 positive tests (specific info / concrete claim / role-anchored advice / real voice), rewrite or delete. Watchlist words themselves are fine when they're load-bearing; the real problem is the empty sentence structure around them. |
| `tmp/*.json` has `ok: false` | Treat that source as untrustworthy — use items from it only if corroborated by another source. If the degraded source is load-bearing for today's narrative, mention the degradation in the lead. |
| Memory file has `schema_version !== 2` | Do not migrate it. Write your Step 9 output in v2 shape anyway; the validator will flag the mismatch and the run aborts — flag it in your Step 10 exit so the operator sees it. |
| Fewer than 3 genuinely non-obvious idea connections | Output fewer ideas rather than forcing filler. `ideas[]` minimum is 1 in the schema. Three forced connections are worse than one real one. |
| A candidate repo is from `memory.short_term.featured_repos[*]` with `times_featured >= 3` | Exclude from `ideas` / `shipped`. It can still appear in `signals` if it's part of a narrative arc. |
| Numbers you'd cite don't exist in the source data | Do not estimate or round "about 1000". Either quote the exact value from the source or omit the claim. |
| You catch yourself writing a sentence that could appear unchanged in any other day's report | Delete it. That is the slop test — no exceptions. |
| Two ideas have nearly identical stacks (e.g. both "RAG + Python + vector DB") | Rework one to use a different primitive, or cut it. Diversity rule (Step 5) requires at least 1 non-AI/dev-tooling idea. |
| Agent prompt and schema disagree about a field | Schema wins. Write what the schema allows, log the disagreement in `memory.open_threads` for the next human touch. |
