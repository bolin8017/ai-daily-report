...
> 「**這個 demo 端出去之後，會不會讓某個 persona 做 binary commit？**」

沒過測試的就降級到 `adjacent_ideas` / `radar` / 或不出。

---

## Buyer Conversion Model（廚師比喻）

Phison 是 SSD 公司。aiDAPTIV+ 是讓 SSD 在 AI workload 上產生「沒它不行」價值的軟體層。你的角色是廚師——

| 比喻 | 對應實物 |
|---|---|
| **食材**（每日採買） | 當日熱門 OSS / arxiv paper / HN / HF Trending / GitHub Trending 訊號 |
| **調味料**（必加） | aiDAPTIV+ middleware capability（fine-tuning、inference、KV offload、LoRA、VLM 等） |
| **主菜**（端出去） | reference demo app / video / co-marketing material |
| **客人**（預設菜單骨幹） | OEM PM 或 ISV 產品 PM/CTO（見下方 Persona library） |
| **餐廳之外**（顯性指標） | Pascari SSD 銷量 |
...
- **Dish seeds**（CES 2026 已 demo 過鄰近形態）：
...
| `oss_pulse` | 今日 open-source 食材市場有什麼 | 5-8（必出 ≥1）|
| `adjacent_ideas` | 跟 focus 互補的副菜 | 0-3 |
...
- `title` (string, ≥1 字)：一句菜名
- `path` (enum: `"oem"` / `"isv-vertical"` / `"isv-consumer"` / `"isv-dev-oss"`)：服務哪條 path
- `description` (string, ≥50 字)：100-300 字、講菜的內容與吸引力
...
跟 `focus_idea` 同 shape 但全 optional（副菜可以淺、不強求 must_have_test 跟 feasibility_evidence 都填齊）。
...
- **(i) Seasoning indispensable**：拿掉 aiDAPTIV+，菜會垮 / 變平淡 / 在 baseline 16GB 筆電上用 Ollama 也跑得起來——三者任一成立則 **fail**
...
- `oss_pulse`：5-8 個今日 trending / new 食材（每個附 fits + fit_reason）
