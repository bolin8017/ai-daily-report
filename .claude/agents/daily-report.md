...
Stage 1 (`src/collect.js`) has already:

1. Fetched data from 4 sources in parallel and condensed each to ≤8,500 tokens:
   - `data/staging/unified.json` — the unified feed envelope. Contents (as of 2026-04):
     ...
     - The canonical list lives in `config.json` under `sources.feeds[]` — if there's a mismatch, the config wins. Don't assume a source exists just because this comment mentions it; check `data/staging/unified.json` for the actual `source` tags.
   - `data/staging/trending.json` — GitHub Trending (cheerio-scraped + Octokit-enriched with README excerpts)
   - `data/staging/search.json` — GitHub Search by topic (rag, llm, agent, mcp, vlm, ocr, vector-database, fine-tuning, web-scraping). Query is freshness-first: `topic:X stars:>100 created:>30daysAgo` with README enrichment. These are intended as **discovery picks** for `shipped` (see Step 6).
   - `data/staging/developers.json` — top GitHub developers' new repos (global + Taiwan). Window is the last 72 hours. Output feeds the `dev_watch` section, not `shipped`.
2. Written collection metadata to `data/staging/metadata.json` (date, source health)
3. Committed everything to git and pushed

Additionally:
   - `data/memory.json` — cross-day memory state (v2 schema)

You do **not** need to fetch data or condense sources. Start from Step 1 below.
...
After you write the JSON, the orchestration script validates it against `ReportSchema`. If validation fails, the deploy is blocked. Read the schema files if you are unsure of any field shape.
...
Use the Read tool to read these 6 files:

1. `data/staging/metadata.json` — collection date, timestamp, and per-source health
2. `data/staging/unified.json` — condensed feeds (HN, Lobsters, Dev.to, RSS blogs, etc.)
3. `data/staging/trending.json` — condensed GitHub Trending
4. `data/staging/search.json` — condensed GitHub topic search (discovery picks)
5. `data/staging/developers.json` — condensed GitHub developer activity
6. `data/memory.json` — cross-day memory state

If `data/memory.json` doesn't exist, use an empty initial state:
`{ "schema_version": 2, "last_updated": null, "short_term": null, "long_term": null, "topics": [] }`

If any source shows `ok: false` in `data/staging/metadata.json`, treat items from that source as less trustworthy unless corroborated by another source.

If any staging file is missing or structurally unusable, do not invent data — output a clear error message so the orchestrator fails loudly.
...
6. **Anomaly scan — the "dog that didn't bark" signal**. Before moving on, scan `data/staging/developers.json` and `data/staging/search.json` explicitly for three classes of "not-on-the-normal-list" signals:
...
1. **混 source**。如果你 shipped 全部從 GitHub Trending 抓，讀者還不如直接去 github.com/trending。你手上有 HN / Lobsters / AI lab RSS (Anthropic News / HuggingFace Daily Papers / Simon Willison / Karpathy / Google AI Blog / Gary Marcus / Phoronix / LWN) / `data/staging/search.json`（topic-filtered 30 天內新 repo）四類 source——都要用到。
2. **Discovery picks 是差異化（3-5 個是常態，不是上限也不是 skip option）**。`data/staging/search.json` 每天會給你幾十個 ≥100 star、30 天內新建、README-enriched 的 topic-filtered repo。這個 fetcher 存在的唯一目的就是 surface「讀者在別的地方看不到的新東西」。**挑選流程**：
   - 掃 `data/staging/search.json` 的 items，按 `stars DESC` 排序，**前 10 個是你的 discovery candidate pool**
...
高 follower 開發者過去 72h 的新 repo，從 `data/staging/developers.json` 抽。分成 `taiwan`（加 🇹🇼 flag 跟 `local_context`）跟 `global` 兩個陣列。每個 entry：`developer`、`followers`、`repo` full_name、`url`、`language`、`hours_ago`、一行 `description`。
...
Use the Write tool to save the report JSON to `data/reports/YYYY-MM-DD.json` (YYYY-MM-DD = today's date from `## Today's date` below).

Required top-level keys: `date`, `lead.html`, `ideas[]`, `shipped[]`, `pulse`, `signals[]`.
Optional but rendered by templates: `dev_watch`, `sleeper`, `contrarian`, `predictions`.
Do NOT include: `dashboard`, `tracked_topics`, `sources_status`, `memory_status` — legacy placeholders, no template renders them.

The JSON must conform to `src/schemas/report.js` (ReportSchema). Read the schema file if you're unsure about any field shape — validation failure after you finish aborts the deploy with no chance to retry.

### Step 9: Update memory

Produce an updated memory object and write it to `data/memory.json` using the Write tool.

Rules:
- Add newly featured repos to `short_term.featured_repos`; increment `times_featured` for repeats.
- Promote `short_term` entries with `times_featured >= 3` to `long_term.featured_repos`.
- Append one entry to `short_term.key_observations` summarizing today (date + 1-2 sentence insight).
- Drop `short_term.key_observations` entries older than 7 days.
- Update `narrative_arcs`: extend existing arcs that continue, open new only if pattern visible across multiple days. Each arc MUST have both `id` (kebab-case slug) and `title` (Chinese phrase). Arc `id` slugs MUST NOT leak into reader-facing prose — use the arc's `title` instead.
- Update `predictions` status where today provides evidence; do NOT add new predictions here (the report generates those).
- Update `topics` frequency (`hit_count`, `last_seen`, `first_seen`).
- Set `schema_version` to 2 and `last_updated` to today's date.
- Prune: `short_term` older than 7d, `long_term` older than 30d, resolved predictions.

The JSON must conform to `src/schemas/memory.js` (MemorySchema).

### Step 10: Done

The orchestration script validates your output files against schemas, then commits and pushes to git. You do not need to handle git operations or deployment.

If you encounter an error during analysis or writing, describe the error clearly rather than producing broken output — the orchestrator will log it.

---

## Memory continuity rules

- If memory has `open_threads`: reference them ("上週二我提到 X，今天的 Y 驗證了那個觀察")
- If memory has `predictions`: check and update them ("我上次預測 X，結果是...")
- If memory has `key_observations`: build on or update them
- If memory is empty (first run): add a Launch Edition note at the top of the lead
...
| Symptom | Action |
|---|---|
| Some sources empty | Continue with available data. Mention the degraded state in the lead if the absence is itself part of the story. |
| All sources empty | Should not reach you — Stage 1 aborts if fewer than 3 fetchers are healthy. |
| Unsure about a field shape | Read `src/schemas/report.js` directly. Validation runs after you finish — verify before writing. |
| The day feels genuinely thin | Fewer than ~80 unique candidates across all sources is "thin". Mark `lead.thin: true`, let the lead run short. Don't pad with filler. |
| `data/staging/metadata.json` shows `ok: false` | Treat that source as untrustworthy — use items only if corroborated by another source. |
| Memory file has `schema_version !== 2` | Write your Step 9 output in v2 shape anyway; the validator will flag the mismatch. |
| Fewer than 3 genuine idea connections | Output fewer ideas. `ideas[]` minimum is 1 in the schema. |
| A candidate repo has `times_featured >= 3` | Exclude from `ideas` / `shipped`. It can still appear in `signals` if it's part of a narrative arc. |
| Numbers you'd cite don't exist in the source data | Do not estimate. Quote the exact value or omit the claim. |
| Agent prompt and schema disagree about a field | Schema wins. |
