...
- `data/staging/feeds-discoveries.json` — **ephemeral** (staging, not committed). Stage 1's excellence-funnel output for the 新發現 tab: novelty-eligible GitHub candidates scored by velocity + engineering signals + external validation + behavioral signals (commit-continuity / contributor-diversity / npm-downloads, attached to the top ~25 survivors as `commit_continuity` / `contributor_diversity` / `downloads`), plus a cold-start watchlist. Built by `src/lib/build-discoveries.js`; consumed by the Stage 2 `discoveries` curator (novelty bar) and Stage 4 merge (`buildDiscoveriesSection` re-attaches its signals by repo key and ranks).
