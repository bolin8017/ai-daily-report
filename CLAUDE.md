...
The pipeline is split into **four stages**, run under Hermes cron (08:30 Asia/Taipei) or local operator control; the active data contract is repo-local staging plus Hermes Wiki context, not repo-local memory.

- **Stage 1 — collect** (`src/collect.js`): pure Node.js — fetches sources in parallel, condenses each to ≤8500 tokens, builds the feeds snapshot, writes condensed data to `data/staging/`. **Does not commit** — staging is local-only; the feeds snapshot it builds is committed later by Stage 4 for the 11ty footer (see "Storage" below). The **native-RSS feed half is ingested from a self-hosted Miniflux** (fed by self-hosted RSSHub; see "Aggregator ingestion" below + `docker/aggregator/`), partitioned by `themes/<theme>/feeds.opml`: feeds listed there come from one Miniflux pull, everything else (HN/Lobsters, the RSSHub-only `dev-to-top`/`anthropic-news`, slow `sk-hynix-news`, and all structured sources — github-*, leaderboards, mops, hf-trending, arxiv) still fetches via its provider chain. If Miniflux is unconfigured (local dev without the stack), collect falls back to chain-fetching everything. GitHub topic search uses a widened net (floor 40 stars, 25 results/topic, 8 rotating topics/day). After recording the star snapshot, Stage 1 runs the **excellence funnel** (`src/lib/build-discoveries.js`) over all collected GitHub items and writes `data/staging/feeds-discoveries.json` (non-fatal if it fails). The excellence score blends velocity / engineering / external-validation with three **behavioral signals** computed for the top ~25 survivors only — commit-continuity (built vs spike-and-abandon), contributor-diversity (team vs one-off, both from `src/lib/github.js` `getRecentCommits`/`getContributors`), and npm weekly-downloads (`src/lib/registry-downloads.js`, public HTTP, no GitHub quota). The enrichment costs ~50 extra GitHub-core calls/day total and is fully fail-soft (missing signals default to 0; omitting the fetchers reproduces the pre-behavioral ranking). This is the sole input to the 新發現 (discoveries) tab: the Stage 2 discoveries curator applies the novelty bar to it, and Stage 4 merge re-attaches its deterministic signals (velocity / excellence / eng) by repo key.
- **Stage 2 — curate** (`scripts/curate.sh`): 4 parallel `claude -p --model claude-haiku-4-5` subprocesses (one per section: discoveries / pulse / market / tech). Each reads its staging slice, applies its prompt at `themes/<ACTIVE_THEME>/sections/<section>/curator.md`, writes validated JSON to `data/staging/curated/<section>.json`. Criticality is owned by `src/pipeline/stages.js`, which declares **all four curators `required`** — the sequencer invokes `curate.sh <section>` once per section and a non-zero exit fails that stage, blocking merge. `curate.sh`'s own `CRITICAL=(discoveries pulse)` list applies only to the batch invocation (`curate.sh` with no arguments, i.e. local `--curate-only` reruns), where a market/tech failure logs degraded instead of aborting. The `discoveries` curator (新發現) is a critical core curator: it applies the novelty bar to Stage 1's excellence-funnel output (`data/staging/feeds-discoveries.json`), emitting `{rising, dev_watch}` picks (merge re-attaches the deterministic funnel signals by repo key and ranks). The earlier optional `curate.catalog` stage + the 精選/上線 tabs were retired on the 2026-06-15 cutover; archived reports still carry `catalog`/`shipped` and render via the templates' legacy conditional.
...
**Production runtime**: production is Hermes cron at 08:30 Asia/Taipei, delivered back to Telegram on failure or notable completion. A second monthly Hermes cron job (1st of month, 05:00 Asia/Taipei) runs `scripts/archive-month.sh` for hot/cold report storage.
...
| `node src/fetchers/miniflux.js` | Any single fetcher/provider can still be run standalone (e.g. `node src/fetchers/providers/github-trending-html.js`); all are dual-mode (importable + CLI). |
...
- **GitHub Trending** (`github-trending-html.js`): scrapes `github.com/trending`, enriches each repo via Octokit. Also one of the GitHub candidate sources feeding the **新發現** excellence funnel — the enrichment must carry `created_at` / `pushed_at`, since the funnel's free gate drops a repo missing them as `too-old` rather than treating them as unknown.
- **GitHub topic search** (`github-search-api.js`): freshness-first `topic:X stars:>100 created:>30daysAgo` + README excerpt; one of the GitHub candidate sources feeding the **新發現** excellence funnel. Topics come from `themes/<theme>/interests.yaml` via `src/lib/interests.js` — each interest declares `level: core` (always on) or `rotating` (date-hash sampled; the search provider samples 8 rotating interests/day, overriding the registry's `rotation.rotating_per_day` default). The `github_topics` list in `sources.yaml` is only the `phison_overlay` scope-tagging input (`src/lib/scope.js`), not the search topic source.
- **GitHub developer watch** (`github-developers-api.js`): top global + per-region (Taiwan) developers' newest **non-fork** repo within a 72h window → feeds the **新發現** funnel's `dev_watch` group (the funnel routes `source: 'github-developers'` items there). It scans the newest 5 rather than the newest 1 — same single listing call — because a fork at the top used to be the only repo seen for that developer, hiding what they actually built (the funnel's `freeGates` rejects forks anyway).
- **Leaderboards / MOPS / HF trending / arXiv**: structured fetchers written raw to staging (no condense step). Leaderboards are **event-driven**: BFCL, LMArena, LiveBench, SWE-bench-Live, GPQA Diamond (Epoch), HLE (Epoch), tau2-bench, GAIA, Artificial Analysis Intelligence Index — each board emits a `tech.benchmarks` item only when its top-5 ranking changed vs the previous run's snapshot (`data/leaderboard-snapshots.json`); frozen or unchanged boards stay silent. GAIA uses `hyparquet` for parquet decode. Artificial Analysis requires `AA_API_KEY` and is fail-soft (no key → silent skip). OCRBench / Aider Polyglot / SWE-bench main board were removed (data stale / never surfaced).
...
- `data/leaderboard-snapshots.json` — per-board ranking snapshot ledger (the cross-day diff backbone for the event-driven 評測 / `tech.benchmarks` tab). Shape: `{ "<bench>": [{ model_id, rank, score, ... }] }`. **Committed by Stage 4** each run (only when populated); **hydrated in CI** from the `data` branch. Same lifecycle as `star-history.json`. Written by `src/lib/leaderboard-snapshots.js`; read by the leaderboard provider to detect ranking changes and suppress unchanged boards.
- `data/staging/feeds-discoveries.json` — **ephemeral** (staging, not committed). Stage 1's excellence-funnel output for the 新發現 tab: novelty-eligible GitHub candidates scored by velocity + engineering signals + external validation + behavioral signals (commit-continuity / contributor-diversity / npm-downloads, attached to the top ~25 survivors as `commit_continuity` / `contributor_diversity` / `downloads`), plus a cold-start watchlist. It also carries `rejected[]` — every repo one of the three gates (free / velocity / engineering) dropped, with the gate, its reason, the `source` intake it came from, and for velocity the full input set (`history_days` / `velocity_per_day` / `total_stars` / `spike` / `has_validation`) so a gate change can be re-judged after the fact; the candidate pool itself lives only in collect's memory and re-running collect samples different topics, so without this a dropped repo left no trace anywhere. `source` is on `candidates` / `watchlist` too, so per-intake yield (how many of an intake's repos survive vs. die at which gate) is answerable from this one file. Built by `src/lib/build-discoveries.js`; consumed by the Stage 2 `discoveries` curator (novelty bar) and Stage 4 merge (`buildDiscoveriesSection` re-attaches its signals by repo key and ranks) — both read only `candidates` / `watchlist`, so `rejected` is diagnostic-only.
...
- **Scheduling**: production runs under Hermes cron at 08:30 Asia/Taipei, with Telegram reporting for failures/notable completion. A separate monthly Hermes cron job (1st of month, 05:00 Asia/Taipei) runs `scripts/archive-month.sh` for hot/cold storage. The repo now owns the production runner (`src/ops/production-run.js`); how Hermes wraps it (thin start/monitor scripts, state contract, migration steps) was documented in [docs/archive/hermes-production-runner.md](./docs/archive/hermes-production-runner.md) (now archived — historical migration guide; the runner itself is `src/ops/production-run.js`).
- **Missed-day detection**: each production run's publish tail diffs the last 14 calendar days against `origin/data:data/reports/` (`src/ops/report-gaps.js`) and surfaces any missing days in the run state (`publish.missing_days`) and the Telegram success/failure notice. A hole means that day's cron never fired (host down) or failed unrecovered. Recover the **same day** with `node src/ops/production-run.js run --state-dir D` (full re-run) or `--recover-from <stage>`; a day discovered missing later cannot be regenerated faithfully — its staging inputs are gone — so the notice's job is to catch the hole while recovery is still possible.
- **Re-provisioning a production host**: before the first production run on a fresh working tree, run `git fetch origin data` so `refs/remotes/origin/data` exists. The cross-day ledger guards (star-history / leaderboard-snapshots / seen-repos) refuse to start from scratch when the branch state is unknowable — without the fetched ref, `recordSnapshot` (star-history) and `appendSeen` (seen-repos) both skip (loudly) on every run and the ledgers never accrue.
- **Degradation reporting**: a run is `succeeded` whenever no *required* stage failed, so `faithfulness` (the only `optional` stage) can degrade — publishing an un-audited editorial — without changing the verdict. Both notices now name any stage that is not `ok`/`skipped` (`degraded: <stage>: <status> — <error>`), not just the failure notice. The `collect` entry in the run state is no longer the sequencer's structurally-constant `skipped`: `collectHealth()` re-derives it from `data/staging/metadata.json` (`sources[].ok` + `degraded`), so a Miniflux outage that drops the whole native-RSS feed half shows up as `collect: degraded — degraded sources: miniflux-feeds` instead of leaving no trace.
