...
- `data/staging/feeds-discoveries.json` — **ephemeral** (staging, not committed). Stage 1's excellence-funnel output for the 新發現 tab: novelty-eligible GitHub candidates scored by velocity + engineering signals + external validation + behavioral signals (commit-continuity / contributor-diversity / npm-downloads, attached to the top ~25 survivors as `commit_continuity` / `contributor_diversity` / `downloads`), plus a cold-start watchlist. It also carries `rejected[]` — every repo one of the three gates (free / velocity / engineering) dropped, with the gate, its reason, and for velocity the full input set (`history_days` / `velocity_per_day` / `total_stars` / `spike` / `has_validation`) so a gate change can be re-judged after the fact; the candidate pool itself lives only in collect's memory and re-running collect samples different topics, so without this a dropped repo left no trace anywhere. Built by `src/lib/build-discoveries.js`; consumed by the Stage 2 `discoveries` curator (novelty bar) and Stage 4 merge (`buildDiscoveriesSection` re-attaches its signals by repo key and ranks) — both read only `candidates` / `watchlist`, so `rejected` is diagnostic-only.
