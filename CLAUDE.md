...
- `data/seen-repos.json` — append-only cross-day dedup ledger for the 精選 catalog section. **Committed by Stage 4** each run alongside the report. Contains the `full_name` of every repo shown in past catalog items; the github-catalog fetcher excludes repos already in this set before building the candidate pool, so the curator only ever sees unseen repos, and Stage 4 (merge) appends the day's shown picks back to the ledger. Mechanical dedup state — distinct from the retired `data/memory.json` LLM intelligence blob.
