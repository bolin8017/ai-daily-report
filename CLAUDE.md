...
- **Re-provisioning a production host**: before the first production run on a fresh working tree, run `git fetch origin data` so `refs/remotes/origin/data` exists. The cross-day ledger guards (star-history / leaderboard-snapshots / seen-repos) refuse to start from scratch when the branch state is unknowable — without the fetched ref, `recordSnapshot` skips (loudly) on every run and the ledgers never accrue.
