...
export async function runFetchers() {
  ...
  // Log per-source errors from feeds fetcher so they're visible in journalctl
  if (result.feeds?.errors?.length) {
    for (const e of result.feeds.errors) {
      console.error(`[fetchers/all] feeds source "${e.feed}" failed: ${e.error}`);
    }
  }
  ...
  // Attach degraded source list to result so staging metadata can surface it
  if (degraded.length) {
