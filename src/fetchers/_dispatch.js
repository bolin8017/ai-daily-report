...
//      src/fetchers/run-all.js and tests.
//   2. A standalone CLI script that writes JSON to stdout and sets exit code
//      based on the `ok` field — preserved so `node src/fetchers/miniflux.js`
//      (or any provider under src/fetchers/providers/) still works for debugging.
//
...
function isEntryPoint(importMetaUrl) {
  ...
  try {
    ...
    console.error(`[dispatch] CLI detection failed: ${err.message}`);
    return false;
  }
}
